...
typedef struct tagSunnyDecData
{
 ...
 void PrintAVFrame(AVFrame* pFrame)
{
	printf(">>  AVFrame: \n");
...
//查找出所有00 00 00 01 这个NALU头位置信息，并且保存到指定文件中
 void LocateNaluHead(unsigned char* h264_buffer, const int h264_len, char* pSaveFile)
{
 ...
 void ParseH264Data(unsigned char* h264_buffer, const int h264_len)
{
    LocateNaluHead(h264_buffer, h264_len, (char*)("./NaluHead.txt"));
}


 int PackingYUV420P(AVFrame* pFrame, void* pOutBuf)
{
